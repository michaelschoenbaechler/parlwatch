...
- `apps/ionic/`: Contains the existing Ionic application workspace
  ...
  - `src/app/`: The main application module and components
    ...
    - `shared/`: Everything features may import from, always through a folder barrel such as `@parlwatch/shared/common/models` (ESLint rejects relative paths and file-level paths; inside a shared area files import each other relatively); features never import each other
